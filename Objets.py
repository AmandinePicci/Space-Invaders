...
# INITIALISATION DES VARIABLES
    ...
    def __init__(self, general, root):      
...
# METHODES 		
    ...
    def Vivre (self):
        ...
        self.toucher = self.canne.find_overlapping(self.canne.coords(self.vaisseau)[0]-10,self.canne.coords(self.vaisseau)[1]-10,self.canne.coords(self.vaisseau)[0]+10,self.canne.coords(self.vaisseau)[1]+10)
        self.toucher = self.toucher[1:len(self.toucher)-1]
        if  type(self.ptsVie)==int and self.ptsVie >=0 and len(self.toucher) > 0:
            self.ptsVie=self.general.MortVaisseau(self.ptsVie, self.root)
            
            
        # Réitère après 2 ms pour continuer le déplacement
        self.canne.after(20, self.Vivre)
